...
const ProductGallery = () => {
    ...
    const removeItem = async(id)=>{
        try {
            ...
            const newProductList = products.filter(product=> product.id!==response.id)
        ...
        <div className='flex justify-center'>
            ...
            <div onMouseLeave={()=> setHoveredItem(null)} className='w-4/6 grid grid-cols-3 grid-rows-3 p-36 gap-8  '>
                {products.map(product=>(
                    <div
                        className=' flex flex-col justify-start items-center p-2 border-2 rounded  shadow-md hover:shadow-2xl relative ' 
                        key={product.id}
                        onMouseEnter={()=>setHoveredItem(product)}
                    >
                        <Link to={`/products/${product.id}`}>
                            <img className='w-36 rounded' src={product.image} alt="" />
                        </Link>
