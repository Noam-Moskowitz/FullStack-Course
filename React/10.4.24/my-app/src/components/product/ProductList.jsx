...
const ProductList = () => {
    ...
    const [response, error, isLoading, fetchData] =useApi(`https://fakestoreapi.com/products`)

    useEffect(() => {
        if (response) {
            const appProducts = response.map(apiProduct=>{
                apiProduct.pName = apiProduct.title;
                delete apiProduct.title;
                return apiProduct
            })

            dispatch({ type: ACTION_TYPES.PRODUCTS_RECEIVED, payload: appProducts }) 
        }
